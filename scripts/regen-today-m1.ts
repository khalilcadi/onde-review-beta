/**
 * Régénère les 3 actions M1 pending du jour avec la version actuelle (RAG M1 désactivé).
 *   - Cyrille GORMAND (Covadia)
 *   - Joël ABREU (Enova)
 *   - JEAN SEBASTIEN WAGNER (Weeflo)
 *
 * Reproduit la logique du cron generate-actions pour M1 :
 *   - parseGenerationResponse(text, true)
 *   - canal === "none" → email_recommended
 *   - sinon variante_a par défaut, humanize, generation_data = parsed.m1
 *
 * Usage:
 *   npx tsx scripts/regen-today-m1.ts            # DRY RUN
 *   npx tsx scripts/regen-today-m1.ts --insert   # remplace les actions pending
 */
...
async function main() {
  console.log(`\n===== REGEN M1 (${INSERT ? "INSERT" : "DRY RUN"}) — 3 leads =====\n`);
  ...
  for (const target of TARGETS) {
    ...
    // M1 = no previous sent messages
    const messageStepNumber = 1;
    ...
    console.log(`  📊 ICP: ${icpSegment ?? "∅"} | Signal: ${signalType ?? "∅"} | Stage: ${lead.stage}`);
    console.log(`  🎯 Séquence: "${seqInfo.name}" | Step ${nextStep.step_order} (${nextStep.step_type}) | M1`);
    ...
    if (!parsed.m1) {
      console.log(`  ⚠️  Parsing M1 a échoué.`);
      console.log(aiResult.text.slice(0, 800));
      continue;
    }

    let actionStatus: string;
    let generatedMessage: string | null;
    const generationReasoning = parsed.m1.reasoning ?? parsed.reasoning ?? null;
    const generationData = parsed.m1 as unknown as Json;

    if (parsed.m1.canal === "none") {
      actionStatus = "email_recommended";
      generatedMessage = null;
      console.log(`\n  📭 Canal = "none" → email recommandé (pas de message LinkedIn)`);
      console.log(`  💡 Reasoning: ${(generationReasoning ?? "").slice(0, 300)}`);
    } else {
      ...
      console.log(`\n  ✏️  M1 variante_a :`);
      console.log("  " + (parsed.m1.variante_a?.message ?? "∅").split("\n").join("\n  "));
      if (parsed.m1.variante_b?.message) {
        console.log(`\n  ✏️  M1 variante_b :`);
        console.log("  " + parsed.m1.variante_b.message.split("\n").join("\n  "));
      }
      console.log(`\n  📊 ${messageText.length} chars | canal=${parsed.m1.canal}`);
      console.log(`  💡 Reasoning: ${(generationReasoning ?? "").slice(0, 300)}`);
    }
    ...
    if (INSERT) {
      ...
      if (insertErr) {
        ...
        console.log(`  ✅ Action remplacée: ${insertData?.id} (status=${actionStatus})`);
      }
    } else {
